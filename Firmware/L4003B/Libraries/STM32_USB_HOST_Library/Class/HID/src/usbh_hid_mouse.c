...
{	
	...
	else if (mouse4 == 2)
	{
		...
		if (data[2] & 0x08)
		{// 负向
			if (HID_MOUSE_Data.x < 0x80)
			HID_MOUSE_Data.x |= 0x80;
		}
		else
		{// 正向
			if (HID_MOUSE_Data.x > 0x80)
			HID_MOUSE_Data.x = 0x7f;// 限制范围
		}
		
		if (data[3] & 0x80)
		{// 负向
			if (HID_MOUSE_Data.y < 0x80)
			HID_MOUSE_Data.y |= 0x80;
		}
		else
		{// 正向
			if (HID_MOUSE_Data.y > 0x80)
			HID_MOUSE_Data.y = 0x7f;// 限制范围
		}
		
		
		//printf("WM ---------------> MOUSE 2 xy: [%02x] [%02x] \r\n",HID_MOUSE_Data.x,HID_MOUSE_Data.y);
		USR_MOUSE_ProcessData(&HID_MOUSE_Data);
