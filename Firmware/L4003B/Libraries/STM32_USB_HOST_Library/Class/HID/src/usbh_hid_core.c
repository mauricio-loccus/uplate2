...
{	
  ...
  if(pphost->device_prop.Itf_Desc[0].bInterfaceSubClass  == HID_BOOT_CODE)
  {
    ...
    printf("      --->  通道in : %d ==> .hc_num_in: %d ,通道out : %d ==> .hc_num_out : %d \r\n",HID_Machine.HIDIntInEp,HID_Machine.hc_num_in,HID_Machine.HIDIntOutEp,HID_Machine.hc_num_out);
		printf("      --->  地址: 0x%08x      speed: %d \r\n",pphost->device_prop.address,pphost->device_prop.speed);
		printf("      --->   InSize : %d   ====  OutSize:%d \r\n" ,HID_Machine.length, HID_Machine.length);
		
		if (HID_Machine.length == 4)
				mouse4 = 1;
		if (HID_Machine.length == 8)
				mouse4 = 2;
...
{   
  ...
  case HID_REQ_SET_IDLE:
    ...
    classReqStatus = USBH_Set_Idle (pdev, pphost, 1000, 0);
...
* @retval USBH_Status
*/
static USBH_Status USBH_HID_Handle(USB_OTG_CORE_HANDLE *pdev , //clase_cb->machine( )
                                   void   *phost)
{
  USBH_HOST *pphost = phost;
