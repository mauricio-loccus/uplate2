...
//		printf("**************************** BulkSendData : OVER ****** 1 len:[%d] hcout:[%d]***********\r\n",USBH_MSC_BOT_CBW_PACKET_LENGTH,MSC_Machine.hc_num_out);
    ...
    case USBH_MSC_BOT_DATAIN_STATE:
      ...
      if((URB_Status == URB_DONE) ||(USBH_MSC_BOTXferParam.BOTStateBkp != USBH_MSC_BOT_DATAIN_STATE))
      {
        ...
        if(remainingDataLength > MSC_Machine.MSBulkInEpSize)//.fmcusb161213
        {
        ...
        {       
          ...
          //printf("*************** BulkReceData : OVER ****** 2 len:[%d] hcin:[%d]***********\r\n",remainingDataLength,MSC_Machine.hc_num_in);
    ...
    case USBH_MSC_BOT_DATAOUT_STATE:
      ...
      if(URB_Status == URB_DONE)
      {
        ...
        if(remainingDataLength > MSC_Machine.MSBulkOutEpSize)
        {
         ...
         // printf("**************************** BulkSendData : OVER ****** 2 len:[%d] hcout:[%d]***********\r\n",MSC_Machine.MSBulkOutEpSize,MSC_Machine.hc_num_out);
        ...
        else
        {
          USBH_BulkSendData (pdev,
			     ...
			     remainingDataLength , 
			     MSC_Machine.hc_num_out);
          //printf("**************************** BulkSendData : OVER ****** 3 len:[%d] hcout:[%d]***********\r\n",remainingDataLength,MSC_Machine.hc_num_out);
          remainingDataLength = 0; /* Reset this value and keep in same state */   
        }      
      }
      ...
      else if(URB_Status == URB_NOTREADY)
      {
        if(datapointer != datapointer_prev)
        {
          USBH_BulkSendData (pdev,
                             ...
                             MSC_Machine.MSBulkOutEpSize , 
                             MSC_Machine.hc_num_out);
					//printf("**************************** BulkSendData : OVER ****** 4 len:[%d] hcout:[%d]***********\r\n",MSC_Machine.MSBulkOutEpSize,MSC_Machine.hc_num_out);
        }
        else
        {
          USBH_BulkSendData (pdev,
                             ...
                             MSC_Machine.MSBulkOutEpSize , 
                             MSC_Machine.hc_num_out);
					//printf("**************************** BulkSendData : OVER ****** 5 len:[%d] hcout:[%d]***********\r\n",MSC_Machine.MSBulkOutEpSize,MSC_Machine.hc_num_out);
        }
      }
    ...
    case USBH_MSC_RECEIVE_CSW_STATE:
      /* BOT CSW stage */     
        ...
        USBH_MSC_BOTXferParam.pRxTxBuff = USBH_MSC_CSWData.CSWArray;// CSWArray 只有13
        USBH_MSC_BOTXferParam.DataLength = USBH_MSC_CSW_MAX_LENGTH; // 长度为 63
