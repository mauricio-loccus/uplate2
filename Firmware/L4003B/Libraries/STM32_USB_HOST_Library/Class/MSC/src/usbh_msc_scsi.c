  ...
  {  
    ...
    case CMD_SEND_STATE:
      ...
      USBH_MSC_CBWData.field.CBWTransferLength = \
                                                ALLOCATION_LENGTH_REQUEST_SENSE; // 63
      USBH_MSC_CBWData.field.CBWFlags = USB_EP_DIR_IN;
      USBH_MSC_CBWData.field.CBWLength = CBW_LENGTH;
      USBH_MSC_BOTXferParam.pRxTxBuff = USBH_DataInBuffer;
      ...
      for(index = CBW_CB_LENGTH - 1; index != 0; index--)//CBW_CB_LENGTH =  16
      {
        USBH_MSC_CBWData.field.CBWCB[index] = 0x00;
      }    
      
      USBH_MSC_CBWData.field.CBWCB[0]  = OPCODE_REQUEST_SENSE; // 0x03
      USBH_MSC_CBWData.field.CBWCB[1]  = DESC_REQUEST_SENSE;// 0x00
      USBH_MSC_CBWData.field.CBWCB[4]  = ALLOCATION_LENGTH_REQUEST_SENSE;// 63
                         ...
                         uint32_t nbOfbytes)
{
  ...
  {  
    ...
    case CMD_SEND_STATE:   
      ...
      USBH_MSC_CBWData.field.CBWCB[0]  = OPCODE_WRITE10; // 0x2A
                        ...
                        uint32_t nbOfbytes)
{
  ...
  if(HCD_IsDeviceConnected(pdev))
  {
    ...
    case CMD_SEND_STATE:
      ...
      for(index = CBW_CB_LENGTH - 1; index != 0; index--) // cbw_cb_length == 16
      {
        USBH_MSC_CBWData.field.CBWCB[index] = 0x00; // CBWCB[16]
      }
      
      USBH_MSC_CBWData.field.CBWCB[0]  = OPCODE_READ10; // 0x28
