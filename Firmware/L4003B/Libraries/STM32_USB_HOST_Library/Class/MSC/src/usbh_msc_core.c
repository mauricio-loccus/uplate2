...
{	 
     ...
     (pphost->device_prop.Itf_Desc[0].bInterfaceProtocol == MSC_PROTOCOL))
  {
    ...
    {
			printf("        1           MSC:  bEndpointAddress & 0x80 == true \r\n");
    ...
    {
			printf("        2           MSC:  bEndpointAddress & 0x80 == false \r\n");
    ...
    {
			printf("        3           MSC:  bEndpointAddress & 0x80 == true \r\n");
    ...
    {
			printf("        4           MSC:  bEndpointAddress & 0x80 == false \r\n");			
    ...
    //{
		//	MSC_Machine.MSBulkInEpSize = 16;
		//	MSC_Machine.MSBulkOutEpSize = 16;
		//}
    ...
    USBH_Open_Channel  (pdev,
		...
		printf("      --->  通道in : %d ==> .hc_num_in: %d ,通道out : %d ==> .hc_num_out : %d \r\n",MSC_Machine.MSBulkInEp,MSC_Machine.hc_num_in,MSC_Machine.MSBulkOutEp,MSC_Machine.hc_num_out);
		printf("      --->  地址: 0x%08x      speed: %d \r\n",pphost->device_prop.address,pphost->device_prop.speed);
		printf("      --->   InSize : %d        OutSize:%d \r\n" ,MSC_Machine.MSBulkInEpSize, MSC_Machine.MSBulkOutEpSize);
...
MSCErrorCount = 0;
    ...
    case USBH_MSC_BOT_USB_TRANSFERS:
      ...
      USBH_MSC_HandleBOTXfer(pdev,phost);
		//printf("**************************** USBH_MSC_Handle :USBH_MSC_BOT_USB_TRANSFERS: USBH_MSC_HandleBOTXfer(pdev,phost)OVER ******\r\n");
      break;
