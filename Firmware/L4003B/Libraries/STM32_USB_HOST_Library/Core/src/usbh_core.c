...
/** @defgroup USBH_CORE_Private_TypesDefinitions
  * @{
  */ 
//extern unsigned char flag_usb_hs;
//extern unsigned  char flag_usb_fs;
...
USBH_HCD_INT_cb_TypeDef  *USBH_HCD_INT_fops = &USBH_HCD_INT_cb; //WM 这个结构的赋值 要注意.....................实际功能在USBH_HCD_INIT_cb 中对应函数
...
unsigned char WMSELE_MSC_HID(USBH_HOST *phost,unsigned char flag);
...
void USBH_Process(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  ...
  /* check for Host port events || (HCD_IsPortEnabled(pdev) == 0)*/
  if ((HCD_IsDeviceConnected(pdev) == 0)&& (phost->gState != HOST_IDLE)) 
  {
    if(phost->gState != HOST_DEV_DISCONNECTED) 
    {
      phost->gState = HOST_DEV_DISCONNECTED;
			//flag_usb_hs=0;//lugj modify 20180211
    }
  }
	//if(*WMnotsup == 1)
	//{
  ...
  case HOST_DEV_ATTACHED :
    ...
    if ( HCD_ResetPort(pdev) == 0)
    {
      ...
      USBH_Open_Channel (pdev,
                         ...
                         EP_TYPE_CTRL,
                         phost->Control.ep0size);          
			printf("    hc_num_in : %d , hc_num_out : %d  \r\n",phost->Control.hc_num_in,phost->Control.hc_num_out);
			printf("    address    : 0x%08x  speed  : %d  \r\n",phost->device_prop.address,phost->device_prop.speed);
			printf("    ep0size    : 0x%08x    \r\n",phost->Control.ep0size);
			printf("                                                                            \r\n");
  ...
  case HOST_ENUMERATION:     
    ...
    { 
      ...
      printf("  -------------------------- WM FS HS 重要提示:枚举后进入了   挂起  \r\n");
  ...
  case HOST_USR_INPUT:    //USB U盘鼠标插入
    /*The function should return user response true to move to class state */
    if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK)// 这个函数是不处理请求等待按钮处理的,直接返回 USBH_USR_RESP_OK
    {
			if(WMSELE_MSC_HID(phost,0))
				{ //0:??MSC,1:??HID
          //SetHOST_USB_MODE(0);
          ...
          phost->class_cb=&USBH_MSC_cb;
					...
					flag_usb_hs=1;////U盘进入这里lugj 20180306		
					//phost->gState = HOST_DEV_ATTACHED;
        }
				else
				{
           if(WMSELE_MSC_HID(phost,1)){ //0:??MSC,1:??HID
              //SetHOST_USB_MODE(1);
						  phost->class_cb->DeInit(pdev,phost);
              ...
              flag_usb_hs=2;//fmcusb			//鼠标进入这里lugj 20180306				
            }
					 //phost->gState = HOST_DEV_ATTACHED;
				}
    }   
...
//	printf("      WM :        1             Proc USB_HS_CORE WM 的Machine 查询 驱动接口:[0x%08x] 主机接口: [0x%08x]\r\n",pdev,phost);
    status = phost->class_cb->Machine(pdev, phost);// WM 此处调用的就是连接通道完成之后的端点处理程序 USBH_HID_Handle() 或者是 USBH_MSC_Handle
    USBH_ErrorHandle(phost, status);
    break;       
...
void USBH_ProcessWM(USB_OTG_CORE_HANDLE *pdev , USBH_HOST *phost)
{
  volatile USBH_Status status = USBH_FAIL;
  ...
  case HOST_USR_INPUT:    
    ...
    if ( phost->usr_cb->UserInput() == USBH_USR_RESP_OK)// 这个函数是不处理请求等待按钮处理的,直接返回 USBH_USR_RESP_OK
    {
          ...
          phost->class_cb=&USBH_MSC_cb;
					...
					flag_usb_fs=1;//fmcusb
					//phost->gState = HOST_DEV_ATTACHED;
        }
				else
				{
           if(WMSELE_MSC_HID(phost,1))
						 { //0:??MSC,1:??HID
              //SetHOST_USB_MODE(1);
						  phost->class_cb->DeInit(pdev,phost);
              phost->class_cb=&HID_cb;
