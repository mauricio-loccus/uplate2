                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  pdev->cfg.dma_enable       = 0;// WM 文档说明:如果HNP被关闭,则OTG必作为主机模式运行
...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (pdev->cfg.phy_itface == USB_OTG_ULPI_PHY)
  {
    ...
    printf("WM 1===================USB_OTG_ULPI_PHY===============================> GREGS->GUSBCFG [%d] \r\n",usbcfg.d32);
  ...
  else /* FS interface (embedded Phy) */
  {
    ...
    usbcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GUSBCFG);;
		printf("WM 2===================embedded Phy===============================> GREGS->GUSBCFG [%d] \r\n",usbcfg.d32);
		printf("WM 2===================embedded Phy=============&pdev->regs.GREGS->GUSBCFG ADDr [0x%08x] \r\n",&pdev->regs.GREGS->GUSBCFG);
		
		
    usbcfg.b.physel  = 1; /* FS Interface */
		//usbcfg.d32 = 0;// WM 修改GUSBCFG
    USB_OTG_WRITE_REG32 (&pdev->regs.GREGS->GUSBCFG, usbcfg.d32);
  ...
  /* case the HS core is working in FS mode */// WM 核心工作在FS 模式
  if(pdev->cfg.dma_enable == 1)
  {
    
    ahbcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GAHBCFG);
		printf("WM 3===================核心工作在FS 模式===============================> GREGS->GUSBCFG [%d] \r\n",usbcfg.d32);
		printf("WM 3===================核心工作在FS 模式=============&pdev->regs.GREGS->GUSBCFG ADDr [0x%08x] \r\n",&pdev->regs.GREGS->GAHBCFG);
...
#ifdef  USE_OTG_MODE
  usbcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GUSBCFG);
	printf("WM 4===================USE_OTG_MODE===============================> GREGS->GUSBCFG [%d] \r\n",usbcfg.d32);
		
  usbcfg.b.hnpcap = 1;// WM 文档说明:如果HNP被关闭,则OTG必作为主机模式运行
  usbcfg.b.srpcap = 1;
  USB_OTG_WRITE_REG32(&pdev->regs.GREGS->GUSBCFG, usbcfg.d32);
...
#if defined (STM32F446xx) || defined (STM32F469_479xx)
  usbcfg.d32 = USB_OTG_READ_REG32(&pdev->regs.GREGS->GUSBCFG);
	printf("WM 5===================USE_OTG_MODE=STM32F446xx=STM32F469_479xx=============================> GREGS->GUSBCFG [%d] \r\n",usbcfg.d32);
...
#ifdef USE_OTG_MODE
//  USB_OTG_OTGCTL_TypeDef          gotgctl; // WM 修改
  ...
  /* Initialize Host Configuration Register */
	if (pdev->cfg.coreID == USB_OTG_FS_CORE_ID) // WM 修改FS -> HOST模式的频率设置
	{
		USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 6000 );// WM 修改 增加 设备为 12 时FS的U盘工作正常  设备为 6 时鼠标枚举正常
  ...
  {
		USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 48000 );// WM 修改 增加
...
//#endif  
  ...
  gotgctl.b.hstsethnpen = 1;// WM 文档说明:如果HNP被关闭,则OTG必作为主机模式运行
...
USB_OTG_STS USB_OTG_CoreInitDev (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  {  
    ...
    /* EP0 TX*/
//    nptxfifosize.b.depth     = TX0_FIFO_FS_SIZE; // WM  由于增加了使能 USB_OTG_FS_CORE = 1 所以在这里的DEVICE配置中已经出问题了,暂时停用
    nptxfifosize.b.startaddr = RX_FIFO_FS_SIZE;
    USB_OTG_WRITE_REG32( &pdev->regs.GREGS->DIEPTXF0_HNPTXFSIZ, nptxfifosize.d32 );
...
//    txfifosize.b.depth = TX1_FIFO_FS_SIZE;       // WM  由于增加了使能 USB_OTG_FS_CORE = 1 所以在这里的DEVICE配置中已经出问题了,暂时停用
    USB_OTG_WRITE_REG32( &pdev->regs.GREGS->DIEPTXF[0], txfifosize.d32 );
    
    
    /* EP2 TX*/
    txfifosize.b.startaddr += txfifosize.b.depth;
//    txfifosize.b.depth = TX2_FIFO_FS_SIZE;       // WM  由于增加了使能 USB_OTG_FS_CORE = 1 所以在这里的DEVICE配置中已经出问题了,暂时停用
    USB_OTG_WRITE_REG32( &pdev->regs.GREGS->DIEPTXF[1], txfifosize.d32 );
    
    
    /* EP3 TX*/  
    txfifosize.b.startaddr += txfifosize.b.depth;
//    txfifosize.b.depth = TX3_FIFO_FS_SIZE;       // WM  由于增加了使能 USB_OTG_FS_CORE = 1 所以在这里的DEVICE配置中已经出问题了,暂时停用
