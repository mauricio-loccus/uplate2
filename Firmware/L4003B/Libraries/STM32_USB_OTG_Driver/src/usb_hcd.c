                  ...
                  USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  USB_OTG_DisableGlobalInt(pdev);// 关闭
  USB_OTG_CoreInit(pdev);

  /* Force Host Mode*/
  USB_OTG_SetCurrentMode(pdev , HOST_MODE); // WM 设定当前主机工作模式为HOST / DEVICE
  USB_OTG_CoreInitHost(pdev);
  USB_OTG_EnableGlobalInt(pdev);//  重新打开
	
	printf("WM ------------------------------------->  HCD_INIT -> 设置完成  \r\n");
#endif
