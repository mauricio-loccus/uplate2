...
//		printf("WM                     0---->       USBH_OTG_ISR_Handler 主控 HOST Mode gintsts.d32: [%d]\r\n",gintsts.d32);

    if (!gintsts.d32)
    {
			printf("WM                     1---->      gintsts.d32 == 0 将返回 0 \r\n");
      return 0;
    }
    
    if (gintsts.b.sofintr)
    {
			printf("WM                     2---->                  ISR 设置SOF ,正常理解,MSC是不需要SOF的,开启应该有问题 \r\n");
      retval |= USB_OTG_USBH_handle_sof_ISR (pdev);
    }
    
    if (gintsts.b.rxstsqlvl)
    {
			//printf("WM                     3---->      USB_OTG_USBH_handle_rx_qlvl_ISR \r\n");
      retval |= USB_OTG_USBH_handle_rx_qlvl_ISR (pdev);
    }
    
    if (gintsts.b.nptxfempty)
    {
			printf("WM                     4---->      USB_OTG_USBH_handle_nptxfempty_ISR \r\n");
      retval |= USB_OTG_USBH_handle_nptxfempty_ISR (pdev);
    }
    
    if (gintsts.b.ptxfempty)
    {
			printf("WM                     5---->      USB_OTG_USBH_handle_ptxfempty_ISR \r\n");
      retval |= USB_OTG_USBH_handle_ptxfempty_ISR (pdev);
    }    
    
    if (gintsts.b.hcintr)
    {
			//printf("WM                     6---->      USB_OTG_USBH_handle_hc_ISR \r\n");
      retval |= USB_OTG_USBH_handle_hc_ISR (pdev);
    }
    
    if (gintsts.b.portintr)
    {
			printf("WM                     7---->      USB_OTG_USBH_handle_port_ISR \r\n");
      retval |= USB_OTG_USBH_handle_port_ISR (pdev);
    }
    
    if (gintsts.b.disconnect)
    {
			printf("WM                     8---->            ISR 设备关闭 ADDr:[0x%08x]\r\n",pdev);
      retval |= USB_OTG_USBH_handle_Disconnect_ISR (pdev);  // WM 设备关闭
      
    }
    
    if (gintsts.b.incomplisoout)
    {
			printf("WM                     9---->      USB_OTG_USBH_handle_IncompletePeriodicXfer_ISR \r\n");
...
uint32_t USB_OTG_USBH_handle_Disconnect_ISR (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  USBH_HCD_INT_fops->DevDisconnected(pdev);// WM 设备关闭
    ...
    if (hprt0.b.prtena == 1)
    {
      ...
      { 
        ...
        if (hprt0.b.prtspd == HPRT0_PRTSPD_LOW_SPEED)
        {
					...
					{
						// 全速 FS 主机运行在低速鼠标时使用 1.5 mhz
						USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 6000 );
						if (hcfg.b.fslspclksel != HCFG_6_MHZ)
            {
							printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                      FS         LOW_SPEED   设置频率: 6MHZ \r\n");
           ...
           if (hcfg.b.fslspclksel != HCFG_6_MHZ)
           {
             ...
             {
							 printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                     HS          LOW_SPEED   设置频率: 6MHZ \r\n");
               USB_OTG_InitFSLSPClkSel(pdev , HCFG_6_MHZ);
             }
            
             else
             {
						 	printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                      HS         LOW_SPEED   设置频率: 3 48 MHZ \r\n");
        ...
        }else if (hprt0.b.prtspd == HPRT0_PRTSPD_FULL_SPEED)
        {
					if (pdev->cfg.coreID == USB_OTG_FS_CORE_ID)
					{
						// 全速 FS 主机使用 12 M
						...
						if (hcfg.b.fslspclksel != HCFG_48_MHZ)
						{
							if(pdev->cfg.phy_itface  == USB_OTG_EMBEDDED_PHY)
							{
								printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                     FS          FULL_SPEED   设置频率: 48 MHZ \r\n");
								USB_OTG_InitFSLSPClkSel(pdev , HCFG_48_MHZ);
							}
							else
							{
								printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                     FS          FULL_SPEED   设置频率:FS-> 12 MHZ \r\n");
								USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 12000 );            
								if (hcfg.b.fslspclksel != HCFG_12_MHZ)
								{
									USB_OTG_InitFSLSPClkSel(pdev ,HCFG_12_MHZ );
								}
							}
					...
					else
					{
						...
						if (hcfg.b.fslspclksel != HCFG_48_MHZ)
						{
							if(pdev->cfg.phy_itface  == USB_OTG_EMBEDDED_PHY)
							{
								printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                       HS        FULL_SPEED   设置频率: 3 48 MHZ \r\n");
								USB_OTG_InitFSLSPClkSel(pdev , HCFG_48_MHZ);
							}
            
							else
							{
								printf("WM                USB_OTG_USBH_handle_port_ISR     ---->                       HS        FULL_SPEED   设置频率: 6MHZ \r\n");
								USB_OTG_WRITE_REG32(&pdev->regs.HREGS->HFIR, 6000 );            
								if (hcfg.b.fslspclksel != HCFG_6_MHZ)
								{
									USB_OTG_InitFSLSPClkSel(pdev ,HCFG_6_MHZ );
