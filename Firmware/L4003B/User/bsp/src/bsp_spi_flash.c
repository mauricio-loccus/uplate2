...
static uint8_t sf_AutoWritePage(uint8_t *_ucpSrc, uint32_t _uiWrAddr, uint16_t _usWrLen);
 uint8_t s_spiBuf[4*1024] __attribute__((at(SDRAM_FLASH_PAGE_BUF)));;// 用于写函数，先读出整个page，修改缓冲区后，再整个page回写 
	...
	/* 自动识别串行Flash型号 */
	{
		...
		switch (g_tSF.ChipID)
		{
			...
			case W25Q64BV_ID:
				...
				g_tSF.PageSize = 4 * 1024;			/* 页面大小 = 4K */
				break;
      case W25Q128fV_ID://0xef4018:
				strcpy(g_tSF.ChipName, "W25Q128FV");
				g_tSF.TotalSize = 16 * 1024 * 1024;	/* 总容量 = 8M *///fmc160911
				g_tSF.PageSize = 4 * 1024;			/* 页面大小 = 4K */
				break;
