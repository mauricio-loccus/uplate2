...
#define RCC_TP_INT	RCC_AHB1Periph_GPIOD
#define PORT_TP_INT	GPIOD
#define PIN_TP_INT	GPIO_Pin_3


//PWR引脚
#define GPIO_PORT_I2C	GPIOA			                 /* GPIO端口 */
#define RCC_PWR_PORT 	RCC_AHB1Periph_GPIOA		   /* GPIO端口时钟 */
#define TOUCH_PWR_PIN		GPIO_Pin_8			         /* 连接到SCL时钟线的GPIO */


static void FT5X06_ReadReg(uint16_t _usRegAddr, uint8_t *_pRegBuf, uint8_t _ucLen);
extern unsigned char tuch_time_over;
extern unsigned int tuch_reset_time;
/*
*********************************************************************************************************
...
void FT5X06_OnePiontScan(void)
{
	...
	uint16_t x, y;
	static uint16_t  go_shake_cut=0;//按键去抖动wwy
  ...
  if(tuch_time_over<200)//fmc161207防止按钮弹不出
		 tuch_time_over++;
	if (FT5X06_PenInt() == 0)
	{
		if(tuch_time_over==30)//fmc161207 //wwy2018.7.5
		{ //tuch_time_over=0;
			//if (s_tp_down == 1)
		  {
				s_tp_down = 0;
		...
		//************************
		//***********************
		go_shake_cut=0;//按键去抖动
		return;
	}//fmc161206 去掉防止按钮弹不回
	else
	{
	  go_shake_cut++;//按键去抖动 ,8ms去抖动
		if(go_shake_cut<3)
		{
		   return;
		}
