...
//#define WAIT_BUSY()	{uint8_t kk;uint16 j; for (kk = 0; kk < 40; kk++);  while( GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_6) == 0 );}


	
/* 逻辑块号映射表。好块总数的2%用于备份区，因此数组维数低于1024。 LUT = Look Up Table */
static uint16_t s_usLUT[NAND_BLOCK_COUNT] __attribute__((at(SDRAM_NAND1_BUF)));       

static uint16_t s_usValidDataBlockCount;	/* 有效的数据块个数 */

uint8_t s_ucTempBuf[NAND_PAGE_TOTAL_SIZE] __attribute__((at(SDRAM_NAND2_BUF)));       //??ABS?
...
/* 必须等待，否则读出数据异常, 此处应该判断超时*/
//0表示忙,1表示就续
uint8_t WAIT_BUSY(void)	
{
     uint8_t kk;
     unsigned short j; 
     //for (kk = 0; kk < 40; kk++);//这里延时时间可修改  
     for (kk = 0; kk < 100; kk++);//这里延时时间可修改  
	   for(j=0;j<10000;j++)
	   {
			    if(GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_6))return 0;
		 }
		 return 1;
...
static void FMC_NAND_Init(void)
{
	...
	/*--NAND Flash GPIOs 配置  ------
		...
		PD6/FMC_INT2	(本例程用查询方式判忙，此口线作为普通GPIO输入功能使用)
	*/
	...
	*/
  	...
  	p.FMC_SetupTime = 0x2; //ComSpaceTiming.SetupTime
...
uint32_t NAND_ReadID(void)
{
  ...
  u32 id;  
	...
	NAND_CMD_AREA = NAND_CMD_READID;//0x90;
	NAND_ADDR_AREA = 0x00;
  	//ID一共有5个字节
...
static uint8_t FMC_NAND_PageCopyBack(uint32_t _ulSrcPageNo, uint32_t _ulTarPageNo)
{
	...
	*/
	NAND_ADDR_AREA = 0;
	NAND_ADDR_AREA = 0;
	NAND_ADDR_AREA = (unsigned char)(_ulSrcPageNo);
	NAND_ADDR_AREA = (unsigned char)((_ulSrcPageNo & 0xFF00) >> 8);

	#if NAND_ADDR_5 == 1
		NAND_ADDR_AREA = (unsigned char)((_ulSrcPageNo & 0xFF0000) >> 16);
	#endif
	...
	*/
	NAND_ADDR_AREA = 0;
	NAND_ADDR_AREA = 0;
	NAND_ADDR_AREA = (unsigned char)_ulTarPageNo;
	NAND_ADDR_AREA = (unsigned char)((_ulTarPageNo & 0xFF00) >> 8);

	#if NAND_ADDR_5 == 1
		NAND_ADDR_AREA = (unsigned char)((_ulTarPageNo & 0xFF0000) >> 16);
	#endif
	...
	uint16_t _usOffset, uint16_t _usSize)
{
	...
	*/
	...
	NAND_ADDR_AREA = (unsigned char)_usOffset;
	NAND_ADDR_AREA = (unsigned char)(_usOffset >> 8);
...
uint8_t FMC_NAND_WritePage(uint8_t *_pBuffer, uint32_t _ulPageNo, uint16_t _usAddrInPage, uint16_t _usByteCount)
{
	...
	/* 发送页写命令 */
	NAND_CMD_AREA = NAND_CMD_WRITE0; //0x80
	...
	/* 发送页内地址 ， 对于 HY27UF081G2A
		...
		第5字节： A28  A29  A30  A31  0    0    0    0
	*/
	NAND_ADDR_AREA = (unsigned char)_usAddrInPage;
	NAND_ADDR_AREA = (unsigned char)(_usAddrInPage >> 8);
	NAND_ADDR_AREA = (unsigned char)_ulPageNo;
	NAND_ADDR_AREA = (unsigned char)((_ulPageNo & 0xFF00) >> 8);

	#if NAND_ADDR_5 == 1
		NAND_ADDR_AREA = (unsigned char)((_ulPageNo & 0xFF0000) >> 16);
	#endif
...
*********************************************************************************************************
*/
uint8_t FMC_NAND_CompPage(uint8_t *_pBuffer, uint32_t _ulPageNo, uint16_t _usAddrInPage, uint16_t _usByteCount)
{
	uint16_t i;
...
*********************************************************************************************************
*/
uint8_t FMC_NAND_ReadSpare(uint8_t *_pBuffer, uint32_t _ulPageNo, uint16_t _usAddrInSpare, uint16_t _usByteCount)
{
	if (_usByteCount > NAND_SPARE_AREA_SIZE)
	{
...
uint8_t FMC_NAND_EraseBlock(uint32_t _ulBlockNo)
{
	...
	*/
	...
	_ulBlockNo <<= 6;	// 块号转换为页编号 
		...
		NAND_ADDR_AREA = (unsigned char)_ulBlockNo;
		NAND_ADDR_AREA = (unsigned char)(_ulBlockNo >> 8);
	#else		/* 512MB的 */
		NAND_ADDR_AREA = (unsigned char)_ulBlockNo;
		NAND_ADDR_AREA = (unsigned char)(_ulBlockNo >> 8);
		NAND_ADDR_AREA = (unsigned char)(_ulBlockNo >> 16);
	#endif
...
static uint8_t NAND_BuildLUT(void)
{
	...
	return NAND_OK;//lgj171115 debug
...
*********************************************************************************************************
*/
uint8_t NAND_MarkUsedBlock(uint32_t _ulBlockNo)
{
	uint32_t ulPageNo;
	uint8_t ucFlag;
...
uint8_t NAND_GetBlockInfo(NAND_BLOCK_INFO_T *_pInfo)
{
	...
	{
		 _pInfo->ChipID = H27U4G8F2DTR;
		strcpy(_pInfo->ChipName, "MT29F4G08ABADA");
	}
	else
	{		
		return 0;
