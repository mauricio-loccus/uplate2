...
/*	#define RCC_SCK 	RCC_AHB1Periph_GPIOB
	...
	#define RCC_MOSI 	RCC_AHB1Periph_GPIOE
	#define PORT_MOSI	GPIOE
	#define PIN_MOSI	GPIO_Pin_6

	#define RCC_MISO 	RCC_AHB1Periph_GPIOE
	#define PORT_MISO	GPIOE
...
void bsp_InitSPIBus(void)
{
	...
	GPIO_Init(GPIOE, &GPIO_InitStructure);//FMCSPI DEBUG
...
*********************************************************************************************************
*/
void bsp_spiDelay(void)  //fmc160830
{
#if 1
	//uint32_t i;
	unsigned char  i;
...
uint8_t bsp_spiRead0(void)
{
	...
	/* 发送一个字节 */
	SPI_I2S_SendData(SPI4, 0);

	/* 等待数据接收完毕 */
	while(SPI_I2S_GetFlagStatus(SPI4, SPI_I2S_FLAG_RXNE) == RESET);

	/* 读取接收到的数据 */
	read = SPI_I2S_ReceiveData(SPI4);
