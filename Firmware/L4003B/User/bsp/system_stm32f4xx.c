...
#if 1   /* 25M¾§Õñ£¬ ÏµÍ³Ö÷Æµ 192M (³¬Æµ)£¬ USB & SDIOÊ±ÖÓ 48M  */
		/* PLL_VCO = (HSE_VALUE or HSI_VALUE / PLL_M) * PLL_N */
		#define PLL_M      25
		/* USB OTG FS, SDIO and RNG Clock =  PLL_VCO / PLLQ */
		#define PLL_Q      7
...
/**
  ...
  * @retval None
  */
static void SetSysClock(void)	//fmc 20160621
{
#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F401xx) || defined(STM32F446xx)|| defined(STM32F469_479xx)
/******************************************************************************/
...
/******************************************************************************/
  ...
  /* Enable HSE */
  RCC->CR |= ((uint32_t)RCC_CR_HSEON);	 //´ò¿ªÍâ²¿¾§ÕñFMC
 
  /* Wait till HSE is ready and if Time out is reached exit */
  do
  {
    HSEStatus = RCC->CR & RCC_CR_HSERDY;   //RCC  Ê±ÖÓ¿ØÖÆ¼Ä´æÆ÷ (RCC_CR) ÖÐµÄ HSERDY ±êÖ¾Ö¸Ê¾¸ßËÙÍâ²¿Õñµ´Æ÷ÊÇ·ñÎÈ¶¨¡£ÔÚÆô¶¯Ê±£¬Ó²¼þ½«´ËÎ»ÖÃ 1 ºó£¬´ËÊ±ÖÓ²Å¿ÉÒÔÊ¹ÓÃ
    StartUpCounter++;
  } while((HSEStatus == 0) && (StartUpCounter != HSE_STARTUP_TIMEOUT));

  if ((RCC->CR & RCC_CR_HSERDY) != RESET)  //=0 
  {
    HSEStatus = (uint32_t)0x01;
  }
  else
  {
    HSEStatus = (uint32_t)0x00;	  //µÃµ½Íâ²¿¾§Õñ×´Ì¬ 1±íÊ¾Õý³£ºÃÓÃ
  }

  if (HSEStatus == (uint32_t)0x01)  //Íâ²¿¾§Õñ×´Ì¬ 1±íÊ¾Õý³£ºÃÓÃ
  {
    /* Select regulator voltage output Scale 1 mode */
    RCC->APB1ENR |= RCC_APB1ENR_PWREN;	 //Ê¹ÄÜµçÔ´½Ó¿ÚÊ±ÖÓ
    PWR->CR |= PWR_CR_VOS;	 //µ÷Ñ¹Æ÷Êä³öµçÑ¹¼¶±ðÑ¡Ôñ

    /* HCLK = SYSCLK / 1*/
    RCC->CFGR |= RCC_CFGR_HPRE_DIV1;  //Ê±ÖÓÅäÖÃ¼Ä´æÆ÷ AHB Ô¤·ÖÆµÆ÷=0²»·Ö£¬(RCC_CFGR) SYSCLK not divided

#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F446xx) || defined(STM32F469_479xx)    
    /* PCLK2 = HCLK / 2*/
    RCC->CFGR |= RCC_CFGR_PPRE2_DIV2;	//429 APB ¸ßËÙÔ¤·ÖÆµÆ÷
    
    /* PCLK1 = HCLK / 4*/
    RCC->CFGR |= RCC_CFGR_PPRE1_DIV4;	//	 APB µÍËÙÔ¤·ÖÆµÆ÷
#endif /* STM32F40_41xxx || STM32F427_437x || STM32F429_439xx  || STM32F446xx || STM32F469_479xx */
...
//**************************************************
#if defined(STM32F40_41xxx) || defined(STM32F427_437xx) || defined(STM32F429_439xx) || defined(STM32F401xx) || defined(STM32F469_479xx)    
    /* Configure the main PLL */										//PLLM £ºÖ÷ PLL (PLL) ºÍÒôÆµ PLL (PLLI2S) ÊäÈëÊ±ÖÓµÄ·ÖÆµÏµÊý 
			// 	 =	8·ÖÆµ   | 336±¶Æµ			|0 | 4m |	 7
//¡ñ f (VCO Ê±ÖÓ) = f (PLL Ê±ÖÓÊäÈë) ¡Á (PLLN / PLLM)
//¡ñ f (PLL ³£¹æÊ±ÖÓÊä³ö) = f (VCO Ê±ÖÓ) / PLLP
//¡ñ f (USB OTG FS, SDIO, RNG Ê±ÖÓÊä³ö) = f (VCO Ê±ÖÓ) / PLLQ

    RCC->PLLCFGR = PLL_M | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) |	//PLLP £ºÊÊÓÃÓÚÖ÷ÏµÍ³Ê±ÖÓµÄÖ÷ PLL (PLL) ·ÖÆµÏµÊý 
                   (RCC_PLLCFGR_PLLSRC_HSE) | (PLL_Q << 24);		   // £ºPLLN £ºÊÊÓÃÓÚ VCO µÄÖ÷ PLL (PLL) ±¶ÆµÏµÊý
																	   // £ºÖ÷ PLL (PLL) ·ÖÆµÏµÊý£¬ÊÊÓÃÓÚ USB OTG FS¡¢SDIO ºÍËæ»úÊý·¢ÉúÆ÷Ê±ÖÓ
	//fmc ÊÇ°ÉÊý¾ÝÍÆµ½¶ÔÓ¦Î»ÉÏ	PLL_M=8  PLL_N 336	PLL_P 2	  RCC_PLLCFGR_PLLSRC_HSE=0x400000  PLL_Q 7
	//RCC_PLLCFGR_PLLSRC_HSE	Ñ¡Ôñ HSE Õñµ´Æ÷Ê±ÖÓ×÷Îª PLL ºÍ ÒôÆµ PLLI2S Ê±ÖÓÊäÈë¡¢¡¢	PLL_Q USB OTG FS Ê±ÖÓÆµÂÊ
