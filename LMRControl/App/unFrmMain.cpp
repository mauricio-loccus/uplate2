...
{
	// Este bloco servir� para a detec��o de um device USB pluggado.
	// No caso espec�fico, servir� para mostrar o cursor do mouse sempre que
	...
	switch (m.WParam)
	{
		...
		case DBT_DEVICEREMOVECOMPLETE:
			...
			else
			{
				...
				throw Exception(TEXT("Comunica��o interrompida, perda de comunica��o com a leitora."));
			}
...
{
	if (mpAppConfig->ElisaSimulated == false)
	{
...
void __fastcall TMainForm::actConnectExecute(TObject *Sender)
{
	try
	{
		...
		// Escrever c�digo para tornar o bot�o toggle e connectar ou desconectar o equipamento
		...
		if (!m_elisaDevice->isAuthenticated)
		{
			...
			MessageDlg(_T("�Plate - Nenhum equipamento compat�vel encontrado."),
		...
		if (!m_elisaDevice->isConnected)
		{
			...
			MessageDlg(System::Sysutils::Format(_T("Erro ao abrir conex�o com a porta \"%s\"."),
...
void __fastcall TMainForm::DoProcessCPnCNs()
{
	...
	if (!cnWells.empty())
	{
		...
		// Equa��es para encontrar os limites
...
void __fastcall TMainForm::actProgramRunExecute(TObject *Sender)
{
	...
	{
		TaskMessageDlg(TEXT("Aten��o"),
					   TEXT("O tempo de dura��o � obrigat�rio quando a \"Agita��o\" est� ativada."),
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
		return;
	}
	...
	{
		TaskMessageDlg(TEXT("Aten��o"),
					   TEXT("Valor para o tempo de agita��o inv�lido"),
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
		return;
	}
	...
	{
		TaskMessageDlg(TEXT("Aten��o"),
					   TEXT("Valor para o tempo de espera inv�lido"),
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
		return;
	}
	...
	if (wlEmpty.size() == m_elisaDeviceParams->PlateRows * m_elisaDeviceParams->PlateCols)
	{
		TaskMessageDlg(TEXT("Aten��o"),
					   TEXT("Placa vazia! Para realizar um experimento, "
							"� necess�rio designar os tipos de leituras na janela \"Placas\"."),
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
	...
	if (!stdWells.empty() && !mCurveImported)
	{
		...
		{
			TaskMessageDlg(TEXT("Aten��o"),
						   TEXT("Foram designados padr�es, por�m nenhum valor foi adicionado.\n"
	...
	else
	{
		...
		{
			TaskMessageDlg(TEXT("Aten��o"),
						   TEXT("N�o � permitido ler uma placa caso hajam \"Padr�es\""
						        " designados e uma curva importada de outro experimento.\n"
								"Para continuar, primeiro remova os padr�es ou inicie um novo Protocolo."),
						   mtWarning,
						   TMsgDlgButtons() << mbOK, 0);
	...
	try
	{
		if (rbKinetic->Checked)
		{
			...
			{
				TaskMessageDlg(TEXT("Aten��o"),
							   TEXT("� necess�rio ao menos um elemento inserido na lista de "
									"intervalos de tempo, quando a leitura cin�tica est� selecionada."),
							   mtWarning,
							   TMsgDlgButtons() << mbOK, 0);
...
void __fastcall TMainForm::DoProcessResults()
{
	...
	/**//************************************************
	 * L�gica para Controle Positivo e Controle Negativo
	 ****************************************************/
	if (!unknownWells.empty())
		DoProcessCPnCNs();

	/**//*************************************
	 * L�gica para Quality Control
	 *****************************************/
	if (!unknownWells.empty())
        ...
        case 1:
            try
            {
				if (0 > stdCurveChart->LeftAxis->Minimum || 0 > stdCurveChart->LeftAxis->Maximum)
				{
					...
					reason.sprintf(TEXT("Para curva Logar�tmica os eixos Y (%5.3f) e "
...
void __fastcall TMainForm::OptUserLoginClick(TObject *Sender)
{
	if (UserLogged())
	{
		MessageDlg("Existe um usu�rio ainda logado. Por favor, fa�a \"Logoff\" antes.",
...
{
	AskDialog->Title = TEXT("Desconex�o do usu�rio ") + mpAppConfig->UserName;
	AskDialog->Content = TEXT("A desconex�o de usu�rio implica na perda de dados.\n")
...
void __fastcall TMainForm::LoadSetupBranch(_di_IXMLNode ProtoNode)
{
    ...
    if (!ReadModeNode || !ReadModeNode->HasChildNodes)
    {
        TaskMessageDlg("Carregamento de Protocolo",
                       "Formato do arquivo inv�lido ou corrompido. Tag \"ReadMode\" n�o encontrada.",
                       mtError,
                       TMsgDlgButtons() << mbOK, 0);
...
void __fastcall TMainForm::CalculateForReplicas(const TWellType type)
{
	...
	for (WellMatrixList::size_type matIndex = 0; matIndex < wellMatrixListRef.size(); matIndex++)
	{
		...
		for (WellList::iterator cit = wl.begin(); cit != wl.end(); cit++)
		{
            ...
            Double minValue = *std::min_element(arrayValues.begin(), arrayValues.end());
            Double maxValue = *std::max_element(arrayValues.begin(), arrayValues.end());
...
void __fastcall TMainForm::acExperimentImportCalibrationExecute(TObject *Sender)
{
    ...
    if (!wl.empty())
    {
		TaskMessageDlg(TEXT("Carregamento da curva de calibra��o"),
					   TEXT("N�o � permitido importar a curva de calibra��o em placas que contenham \"Padr�es\".\n")
                       TEXT("Para continuar, remova os padr�es da placa e tente novamente."),
                       mtWarning,
                       TMsgDlgButtons() << mbOK, 0);
...
//
	...
	FileOpenDialog->Title = TEXT("Carregar curva de calibra��o");
	...
	{
		TaskMessageDlg(TEXT("Carregamento da curva de calibra��o"),
					   TEXT("Formato do arquivo inv�lido ou corrompido. Tag \"Experiment\" n�o encontrada."),
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	{
		TaskMessageDlg(TEXT("Carregamento da curva de calibra��o"),
					   TEXT("O experimento n�o cont�m curva de calibra��o."),
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	{
		TaskMessageDlg(TEXT("Carregamento da curva de calibra��o"),
					   TEXT("Formato do arquivo inv�lido ou corrompido. Tag \"CreationDate\" n�o encontrada."),
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	AskDialog->Title = TEXT("Carregamento da curva de calibra��o");
	AskDialog->Content = TEXT("A curva de calibra��o que est� sendo importada foi criada em ") +
						 TimestampNode->Text +
						 TEXT(".\nOs resultados correm o risco de n�o serem precisos, deseja continuar?");
...
void __fastcall TMainForm::stdValuesGridSetEditText(TObject *Sender, int ACol, int ARow, const UnicodeString Value)
{
	if (!stdValuesGrid->EditorMode)
	{
		...
		{
			TaskMessageDlg(TEXT("Valor n�o permitido"),
						   TEXT("Por favor verifique o valor inserido, os valores devem ser num�ricos com ponto decimal."),
						   mtError,
