...
//	currCommand = ElisaCommands::ElisaCommand_RunProgram;
	...
	unsigned char speedRead   = params->ReadSpeed;  // 1 = standard; 0 = Fast
...
void __fastcall TLMR962023Device::OnReceive(TObject *Sender, AnsiString Buffer)
{
	...
	// Le o buffer todo
	Integer bufferLen = MyDataModule->ZylSerialPort->GetBufferSize();//Buffer.Length();
	if (bufferLen < 20){
		...
		switch (receivedCmd)
		{
			...
			case COMMAND_MEASURE:
			{
				...
				if (filterUsed >= 2 && qtdBytes > idxDado+2) {
					for (RawDataMatrix::size_type row = 0, idx; row < rawDataFilter2.size(); row++)
					{
						for (ColumnsData::size_type col = 0; col < rawDataFilter2[row].size(); col++, idx++)
						{
							union{
								unsigned char charValue[4];
								float floatValue;
