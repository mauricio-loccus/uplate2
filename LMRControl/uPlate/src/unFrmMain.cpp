...
void __fastcall TMainForm::DoProcessBlanks()
{
	...
	if (blankWells.empty())
	{
		for (WellMatrixList::size_type i = 0; i < wellMatrixListRef.size(); ++i)
		{
			...
			for (Integer row = 0; row < m_elisaDeviceParams->PlateRows; row++)       //exibe corretamente os dados linha/coluna ap�s leitura
			{
...
void __fastcall TMainForm::actProgramRunExecute(TObject *Sender)
{
	...
	{
		TaskMessageDlg(L"Atenção",
					   L"O tempo de duração é obrigatório quando a \"Agitação\" está ativada.",
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
		return;
	}
	...
	{
		TaskMessageDlg(L"Atenção",
					   L"Valor para o tempo de agitação inválido",
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
		return;
	}
...
//	}
	...
	if (wlEmpty.size() == m_elisaDeviceParams->PlateRows * m_elisaDeviceParams->PlateCols)
	{
		TaskMessageDlg(L"Atenção",
					   ...
					   L"é necessário designar os tipos de leituras na janela \"Placas\".",
					   mtWarning,
					   TMsgDlgButtons() << mbOK, 0);
	...
	if (!stdWells.empty() && !mCurveImported)
	{
		...
		{
			TaskMessageDlg(L"Atenção",
						   L"Foram designados padrões, porém nenhum valor foi adicionado.\n"
	...
	else
	{
		...
		{
			TaskMessageDlg(L"Atenção",
						   L"Não é permitido ler uma placa caso hajam \"Padrões\""
						   L" designados e uma curva importada de outro experimento.\n"
						   L"Para continuar, primeiro remova os padrões ou inicie um novo Protocolo.",
						   mtWarning,
						   TMsgDlgButtons() << mbOK, 0);
	...
	try
	{
		if (rbKinetic->Checked)
		{
			...
			{
				TaskMessageDlg(L"Atenção",
							   L"É necessário ao menos um elemento inserido na lista de "
							   L"intervalos de tempo, quando a leitura cinética está selecionada.",
							   mtWarning,
							   TMsgDlgButtons() << mbOK, 0);
...
for (Integer col = 0; col < m_elisaDeviceParams->PlateCols; ++col)
{
	...
	{
		for (Integer col = 0; col < m_elisaDeviceParams->PlateCols; col++)      //verificar se h� problema no linha/coluna
		{
...
void __fastcall TMainForm::cbFilter2Change(TObject *Sender)
{
	...
	{
		TaskMessageDlg(L"Valor não permitido",
...
void __fastcall TMainForm::OnLMR96ReadDone(TObject *Sender, const RawDataMatrix& matrix, const RawDataMatrix& matrixFilter2)
{
	...
	for (RawDataMatrix::size_type row = 0; row < m_elisaDeviceParams->PlateRows; row++)     //exibe corretamente os dados linha/coluna ap�s leitura
	{
...
void __fastcall TMainForm::tabStandardsBeforeShowPage(TObject *Sender)
{
	...
	if (mCurveImported)
	{
		...
		for (cit = mCalibrationCurve->stdValues.begin(); cit != mCalibrationCurve->stdValues.end(); ++cit, row++)  //exibe corretamente os dados linha/coluna ap�s leitura
		{
	...
	for (WellMatrixList::size_type matIndex = 0; matIndex < wellMatrixListRef.size(); matIndex++)
	{
		...
		for (WellList::const_iterator cit = wl.begin(); cit != wl.end(); ++cit, row++)   //exibe corretamente os dados linha/coluna ap�s leitura
		{
        ...
        case 1:
            try
            {
				if (0 > stdCurveChart->LeftAxis->Minimum || 0 > stdCurveChart->LeftAxis->Maximum)
				{
					...
					reason.sprintf(L"Para curva Logar�tmica os eixos Y (%5.3f) e "
        ...
        case 2:
            try
			{
				if (0 > stdCurveChart->LeftAxis->Minimum || 0 > stdCurveChart->LeftAxis->Maximum)
				{
					...
					reason.sprintf(L"Para curva Logarítmica os eixos Y (%5.3f) e "
								   L"X (%5.3f) devem conter valore maiores de 0.",
										stdCurveChart->LeftAxis->Minimum, stdCurveChart->LeftAxis->Maximum);
...
void __fastcall TMainForm::LoadPlatesBranch(_di_IXMLNode PlatesNode)
{
	...
	while (plateNode)
	{
		...
		for (Integer row = 0; row < m_elisaDeviceParams->PlateRows;  row++)      //itera��o para preencher std values do experimento
		{
...
//
	...
	{
		TaskMessageDlg(L"Carregamento da curva de calibração",
					   L"Formato do arquivo inválido ou corrompido. Tag \"Experiment\" não encontrada.",
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	{
		TaskMessageDlg(L"Carregamento da curva de calibração",
					   L"O experimento não contém curva de calibração.",
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	{
		TaskMessageDlg(L"Carregamento da curva de calibração",
					   L"Formato do arquivo inv�lido ou corrompido. Tag \"CreationDate\" não encontrada.",
					   mtError,
					   TMsgDlgButtons() << mbOK, 0);
	...
	AskDialog->Title   = L"Carregamento da curva de calibração";
	AskDialog->Content = L"A curva de calibração que está sendo importada foi criada em " +
						 TimestampNode->Text +
...
void __fastcall TMainForm::acExportRawValuesExecute(TObject *Sender)
{
	...
	// Se o arquivo j� existe, deletamos para evitar sobrescrita errada
	if (FileExists(rawFileName))
		DeleteFile(rawFileName);

	// Abrir o arquivo uma �nica vez em modo de adi��o (fmOpenWrite | fmShareDenyWrite)
	...
	for (plateIdx = 0; plateIdx < wellMatrixListRef.size(); plateIdx++)
	{
		...
		// Adiciona as informa��es extras ap�s a tabela
		...
		// Escreve no arquivo (sem reabrir em cada itera��o!)
		for (std::vector<String>::size_type i = 0; i < vectCSVList.size(); i++)
			fStream->WriteLine(vectCSVList[i]);
	...
	// Fecha o arquivo ap�s todo o loop
	fStream->Close();
	fStream->BaseStream->Free();
	fStream->Free();
